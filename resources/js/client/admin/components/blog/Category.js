...
import { Drawer, Button, Spin, Input, Form, Switch } from 'antd';
...
const Category = (props) => {
    ...
    useEffect(() => {
        form.setFieldsValue({
            ...
            description: props.itemToEdit ? props.itemToEdit.description : '',
            is_active: props.itemToEdit ? Boolean(props.itemToEdit.is_active) : true
        });
    }, [props.itemToEdit]);
    ...
    const handleOk = () => {
        ...
        .then((values) => {
            ...
            HTTP[values.id ? 'put' : 'post'](Routes.api.admin.blogCategories + (values.id ? `/${values.id}` : ''), {
                id: values.id,
                name: values.name,
                slug: values.slug,
